{{ config(materialized='incremental', unique_key='transfer_key') }}

{% if is_incremental() %}
WITH max_transfer AS (
    SELECT MAX(transfer_date_key) AS max_dt FROM {{ this }}
)
{% endif %}
...
{% if is_incremental() %}
WHERE dd.date_key > (SELECT max_dt FROM max_transfer)
{% endif %}